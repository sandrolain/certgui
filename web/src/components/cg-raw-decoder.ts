...
export class CgRawDecoder extends LitElement {
  ...
  override render() {
    if (!this.file) {
      return html`
        <div class="text-base-content/40 text-sm p-4">No file selected.</div>
      `;
    }
      ...
      <div class="space-y-2">
        <div class="flex gap-1">
          ...
          <span class="ml-auto text-xs text-base-content/40"
            >${this.file.name} (${this.file.size} bytes)</span
          >
        </div>
        ${this._loading
          ? html`<span class="loading loading-spinner loading-sm"></span>`
          : html`<pre
              class="raw-content bg-base-200 rounded p-3 max-h-96 overflow-auto"
            >
${this._content}</pre
            >`}
      </div>
